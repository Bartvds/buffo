...
function getChunkReader(callback) {
	...
	var reader = valueReader(mill, source, callback);
...
function encodeStream() {
	...
	// keep a small buffer to bunch many small fragments into larger chunks
	function push(chunk) {
		if (length + chunk.length > limit && length > 0) {
			transform.push(Buffer.concat(buffers));
			buffers = [];
			length = 0;
		}
		buffers.push(chunk);
