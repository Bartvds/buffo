...
_nan.writeUInt8(NAN);
_infPos.writeUInt8(INF_POS);
_infNeg.writeUInt8(INF_NEG);
...
/*
function parseValueX(source, pointer, push) {
	var type = source.readUInt8(pointer);
	pointer += 1;
	switch (type) {
		case STRING:
			pointer = stringParse(source, pointer, push);
			break;
		case NUMBER:
			pointer = numberParse(source, pointer, push);
			break;
		case TRUE:
			push(true);
			break;
		...
		case ARGUMENTS:
		case ARRAY:
			pointer = arrayParse(source, pointer, push);
			break;
		case OBJECT:
			pointer = objectParse(source, pointer, push);
			break;
		case BUFFER:
			pointer = bufferParse(source, pointer, push);
			break;
		case DATE:
			pointer = dateParse(source, pointer, push);
			break;
		case REGEXP:
			pointer = regexParse(source, pointer, push);
			break;
		case UNDEFINED:
			push(undefined);
			break;
		case NAN:
			...
			break;
		case INF_NEG:
			push(Number.NEGATIVE_INFINITY);
			break;
		case INF_POS:
			push(Number.POSITIVE_INFINITY);
			break;
	}
	return pointer;
}*/
